...
// IR_List_Table Class
//=============================================
class IR_Lead_List_Table extends WP_List_Table {
    ...
    function column_tag_text ( $item )
    {
        //Build row actions
        $actions = array(
            'edit'    => sprintf('<div style="clear:both;"></div><a href="?page=%s&action=%s&lead_list=%s">'. __('Edit', 'inbound-rocket').'</a>', esc_attr($_REQUEST['page']), 'edit_list',$item['tag_id']),
            'delete'  => sprintf('<a href="?page=%s&action=%s&lead_list=%s">'. __('Delete', 'inbound-rocket').'</a>',esc_attr($_REQUEST['page']),'delete_list',$item['tag_id'])
        );
        
        //Return the title contents
        return sprintf('%1$s<br/>%2$s',
            /*$1%s*/ sprintf('<a class="row-title" href="?page=%s&action=edit_list&lead_list=%s">%s</a>', esc_attr($_REQUEST['page']), $item['tag_id'], $item['tag_text']),
    ...
    function get_lead_lists ()
    {
        ...
        if ( count($tags) )
        {
	        foreach ( $tags as $key => $tag ) 
            {
                ...
                $tag_array = array(
                    'tag_id' => $tag->tag_id,
                    'tag_count' => sprintf('<a href="?page=%s&contact_type=%s">%d</a>', 'inboundrocket_contacts', $tag->tag_slug, ( $tag->tag_count ? $tag->tag_count : 0 )),
    ...
    {
        $orderby = ( !empty($_REQUEST['orderby']) ? esc_attr($_REQUEST['orderby']) : 'tag_order' );
