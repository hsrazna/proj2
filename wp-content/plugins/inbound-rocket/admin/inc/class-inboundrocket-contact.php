...
class IR_Contact {
	...
	{	
		...
		$lead 			= $this->get_contact_details($hashkey);	
		$pageviews 		= $this->get_contact_pageviews($hashkey, 'ARRAY_A');
		$submissions 	= $this->get_contact_submissions($hashkey, 'ARRAY_A');
		$shares			= $this->get_contact_shares($hashkey, 'ARRAY_A');
		$lead_lists 	= $this->get_contact_lead_lists($hashkey);
		...
		$lead->total_visits 		= empty($total_visits) ? 0 : $total_visits;
		$lead->total_pageviews 		= empty($total_pageviews) ? 0 : $total_pageviews;
		$lead->total_submissions 	= empty($total_submissions) ? 0 : $total_submissions;
		$lead->total_shares			= empty($total_shares) ? 0 : $total_shares;

		$this->history 				= (object)NULL;
		$this->history->submission 	= empty($submissions[0]) ? 0 : $submissions[0];
		...
		foreach ( $tags as $tag )
		{
			...
			if ( $tag->tag_synced_lists && $update_tag && ( $tag->tag_relationship_deleted || ! $tag->tag_set ) )
			{
				foreach ( unserialize($tag->tag_synced_lists) as $list )
				{
					...
					$power_up_global = 'inboundrocket_' . $list['esp'] . '_connector_wp';
					...
					if ( array_key_exists($power_up_global, $GLOBALS) )
					{
						...
						$parameters = array(
							...
							'email' => isset($this->history->lead->lead_email) ? $this->history->lead->lead_email : NULL,
							'first_name' => isset($this->history->lead->lead_first_name) ? $this->history->lead->lead_first_name : NULL,
							'last_name' => isset($this->history->lead->lead_last_name) ? $this->history->lead->lead_last_name : NULL,
							'phone' => isset($this->history->lead->lead_phone) ? $this->history->lead->lead_phone : NULL
						);
						
						$contact_synced = ${$power_up_global}->push_contact_to_list($parameters);
					}
