...
class IR_Lead_List_Editor {
	...
	function get_tag_details ( $lead_list_id )
	{
		...
		$q = $wpdb->prepare("SELECT * FROM {$wpdb->ir_tags} WHERE tag_id = %d", $this->lead_list_id);
	...
	function get_form_selectors ( )
	{
		...
		$q = "SELECT COUNT(form_selector_classes) AS freq, form_selector_classes FROM {$wpdb->ir_submissions} WHERE form_selector_classes != '' GROUP BY form_selector_classes ORDER BY freq DESC";
		...
		$q = "SELECT COUNT(form_selector_id) AS freq, form_selector_id FROM {$wpdb->ir_submissions} WHERE form_selector_id != '' GROUP BY form_selector_id ORDER BY freq DESC";
	...
	function add_list ( $tag_text, $tag_form_selectors, $tag_synced_lists )
	{
		...
		$q = "SELECT MAX(tag_order) FROM {$wpdb->ir_tags}";
		$tag_order = $wpdb->get_var($q);
		$tag_order = ( $tag_order ? $tag_order + 1 : 1 );
		$tag_slug = $this->generate_slug($tag_text);

		$q = $wpdb->prepare("
			INSERT INTO $wpdb->ir_tags ( tag_text, tag_slug, tag_form_selectors, tag_synced_lists, tag_order )
	...
	function save_list ( $lead_list_id, $tag_text, $tag_form_selectors, $tag_synced_lists )
	{
		...
		$q = $wpdb->prepare("
			...
			WHERE tag_id = %d", $tag_text, $tag_slug, $tag_form_selectors, $tag_synced_lists, $lead_list_id);	
		$result = $wpdb->query($q);
	...
	function generate_slug ( $tag_text, $lead_list_id = 0 )
	{
		...
		while ( $slug_used )
		{
			...
			$q = $wpdb->prepare("SELECT tag_slug FROM {$wpdb->ir_tags} WHERE tag_slug = %s " . ( $lead_list_id ? $wpdb->prepare(" AND tag_id != %d ", $lead_list_id) : '' ), $tag_slug_modified);
	...
	{
		global $wpdb;
		$q = $wpdb->prepare("SELECT contact_hashkey, lead_email, lead_first_name, lead_last_name FROM {$wpdb->ir_tag_relationships} ltr, {$wpdb->ir_leads} ll WHERE ltr.contact_hashkey = ll.hashkey AND ltr.tag_id = %d AND ltr.tag_relationship_deleted = 0", $lead_list_id);
